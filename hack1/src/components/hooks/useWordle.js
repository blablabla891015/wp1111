...
const useWordle = (solution) => {
    ...
    const handleEnter = () => {
        ...
        newguesses[turn]=curGuess.split('')
        ...
        let solution_set= solution.split('')
        let solution_set2=[]
        console.log(solution_set)
        for(let i=0;i<curGuess.length;i++){
            if(solution_set[i]===curGuess[i]){
                usedChars[solution_set[i]]="green"
            }
            else{
                solution_set2.push(solution_set[i])
            }
        }
        for(let i=0;i<solution_set2.length;i++){
            for(let j=0;j<curGuess.length;j++){
                if(solution_set2[i]===curGuess[j]){
                    usedChars[solution_set2[i]]="yellow"
                }
            }
        }
